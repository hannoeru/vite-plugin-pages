...
const sveltekitRoot = resolve('./examples/sveltekit-style')

const srcPath = resolve('./test/data/test.vue')
const distPath = resolve('./examples/sveltekit-style/src/pages/test/+page.vue')

describe('sveltekit e2e test', () => {
  let server: ViteDevServer
  let browser: Browser
  let page: Page

  beforeAll(async() => {
    server = await createServer(getViteConfig(sveltekitRoot))
